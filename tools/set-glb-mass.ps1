...
$jsonText = [Text.Encoding]::UTF8.GetString($bytes, 20, $jsonLength).TrimEnd(" ", [char]0)
$document = $jsonText | ConvertFrom-Json
$updatedNodes = 0
foreach ($node in $document.nodes) {
    if ($null -eq $node.extras) { continue }
    if ($null -eq $node.extras.physical_material) { continue }
    $node.extras | Add-Member -NotePropertyName mass -NotePropertyValue $MassKg -Force
    $node.extras.PSObject.Properties.Remove("mass_mode")
    $node.extras.PSObject.Properties.Remove("mass_kg")
    $updatedNodes++
}
if ($updatedNodes -eq 0) {
    throw "Nenhum node com physical_material foi encontrado em: $resolvedPath"
}
...
try {
    ...
    $writer.Write([uint32]0) # Preenchido depois de copiar todos os chunks.
    $writer.Write([uint32]$paddedJsonLength)
    $writer.Write([uint32]0x4E4F534A)
    $writer.Write($newJsonBytes)
    for ($index = $newJsonBytes.Length; $index -lt $paddedJsonLength; $index++) {
        $writer.Write([byte]0x20)
    }

    $remainingOffset = 20 + $jsonLength
    if ($remainingOffset -lt $bytes.Length) {
        $writer.Write($bytes, $remainingOffset, $bytes.Length - $remainingOffset)
    }
    $writer.Flush()
    $output = $stream.ToArray()
    [BitConverter]::GetBytes([uint32]$output.Length).CopyTo($output, 8)

    $temporaryPath = $resolvedPath + ".mass.tmp"
...
Write-Output "$([IO.Path]::GetFileName($resolvedPath)): mass=$MassKg kg ($updatedNodes node(s))"
